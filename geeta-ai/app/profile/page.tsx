...
import {
  ...
  AlertCircle, ArrowLeft, Save, RefreshCw, LogIn
} from "lucide-react";
...
function InputField({
  ...
  return (
    <div>
      <label className="mb-1.5 block text-xs font-semibold uppercase tracking-[0.22em] text-white/50" htmlFor={id}>
        {label}
      </label>
      <div className={`flex items-center overflow-hidden rounded-2xl border transition ${error ? "border-red-400/60 bg-red-500/5" : "border-white/12 bg-white/[0.04] focus-within:border-yellow-400/60 focus-within:ring-2 focus-within:ring-yellow-400/10"}`}>
        <span className="flex h-12 w-12 shrink-0 items-center justify-center text-white/36">{icon}</span>
        <input
          className="flex-1 bg-transparent py-3 pr-2 text-sm text-white outline-none placeholder:text-white/28 disabled:opacity-50"
          disabled={disabled}
          id={id}
...
function Toast({ msg, type }: { msg: string; type: "success" | "error" }) {
  return (
    <motion.div
      animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} initial={{ opacity: 0, y: -20 }}
      className={`fixed right-4 top-4 z-50 flex items-center gap-2.5 rounded-2xl border px-5 py-3.5 text-sm font-semibold shadow-xl backdrop-blur ${type === "success" ? "border-teal-400/40 bg-teal-900/80 text-teal-200" : "border-red-400/40 bg-red-900/80 text-red-200"}`}
    >
...
// ─── Avatar Upload ────────────────────────────────────────────────────────────
function AvatarUpload({ src, onChange }: { src: string; onChange: (base64: string) => void }) {
  ...
  async function process(file: File) {
    setErr("");
    if (!ACCEPTED.includes(file.type)) { setErr("Only JPG or PNG allowed."); return; }
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      <div
        className={`relative h-28 w-28 cursor-pointer overflow-hidden rounded-full border-2 transition ${dragOver ? "border-yellow-400 scale-105" : "border-white/20 hover:border-yellow-400/50"}`}
      ...
      <div className="text-center">
        <p className="text-xs text-white/46">Click or drag to change photo</p>
...
export default function ProfilePage() {
  ...
  async function handleSubmit(e: FormEvent) {
    e.preventDefault();
    const errs = validateForm(form, changePassword || isNew);
    ...
    try {
      ...
      setIsNew(false);
      showMsg(isNew ? "Profile created! Welcome 🙏" : "Profile updated successfully!");
    } catch {
  ...
  function handleGoogleSignIn() {
    showMsg("Google Sign-In requires OAuth credentials. Add GOOGLE_CLIENT_ID to .env.local", "error");
  }
    ...
    <div className="min-h-screen bg-[#05020d] font-['Inter',sans-serif] text-white">
      ...
      <div className="relative z-10 mx-auto max-w-2xl px-4 py-10">
        ...
        <div className="mb-8 text-center">
          ...
          <h1 className="mt-3 text-4xl font-bold text-white">{isNew ? "Create Profile" : "My Profile"}</h1>
          <p className="mt-3 text-white/52">
            {isNew ? "Set up your Geeta AI profile to personalise your spiritual journey." : "Manage your personal details and account settings."}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="space-y-6">
            ...
            <motion.div
              ...
              initial={{ opacity: 0, y: 12 }} transition={{ duration: 0.4 }}
            >
            ...
            {/* Personal Info */}
            <motion.div
              animate={{ opacity: 1, y: 0 }} className="rounded-[2rem] border border-white/8 bg-white/[0.03] p-6 space-y-5"
              initial={{ opacity: 0, y: 12 }} transition={{ duration: 0.4, delay: 0.05 }}
            >
              ...
              />
              <InputField
                error={errors.username} icon={<span className="text-xs text-white/40 font-bold">@</span>} id="username" label="Username / User ID"
                onChange={(v) => patch("username", v)} placeholder="e.g. arjuna_seeker" value={form.username}
              />
              <InputField
                error={errors.email} icon={<Mail className="h-4 w-4" />} id="email" label="Email Address"
              ...
              {profile?.authProvider === "google" && (
                <p className="text-xs text-white/40">Email is managed by your Google account.</p>
              )}
            </motion.div>
            ...
            {profile?.authProvider !== "google" && (
              <motion.div
                animate={{ opacity: 1, y: 0 }} className="rounded-[2rem] border border-white/8 bg-white/[0.03] p-6 space-y-5"
                initial={{ opacity: 0, y: 12 }} transition={{ duration: 0.4, delay: 0.1 }}
              >
                <div className="flex items-center justify-between">
                  <h2 className="text-base font-semibold text-white">Password</h2>
                  {!isNew && (
                    <button
                      className="text-xs font-semibold text-yellow-400/80 transition hover:text-yellow-300"
                ...
                {(changePassword || isNew) ? (
                  <>
                    <InputField
                      ...
                      right={
                        <button className="text-white/40 transition hover:text-white" onClick={() => setShowPwd((v) => !v)} type="button">
                          {showPwd ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                        </button>
                      }
                    />
                    <InputField
                      error={errors.confirmPassword} icon={<Lock className="h-4 w-4" />} id="confirmPassword" label="Confirm Password"
                      onChange={(v) => patch("confirmPassword", v)} placeholder="Repeat your password" type={showConfirm ? "text" : "password"} value={form.confirmPassword}
                      right={
                        <button className="text-white/40 transition hover:text-white" onClick={() => setShowConfirm((v) => !v)} type="button">
                      ...
                      <div>
                        <div className="flex gap-1">
                          {[1, 2, 3, 4].map((l) => {
                            ...
                            return (
                              <div className={`h-1.5 flex-1 rounded-full transition ${strength >= l ? (strength === 4 ? "bg-teal-400" : strength >= 3 ? "bg-yellow-400" : "bg-orange-400") : "bg-white/10"}`} key={l} />
                            );
                          })}
                        </div>
                        <p className="mt-1 text-xs text-white/40">
                          Strength: {[form.password.length >= 8, /[A-Z]/.test(form.password), /[0-9]/.test(form.password), /[^a-zA-Z0-9]/.test(form.password)].filter(Boolean).length >= 4 ? "Strong 🔒" : "Add uppercase, number, symbol"}
                        </p>
                      </div>
                    )}
                  </>
                ) : (
                  <p className="text-sm text-white/40">Password is saved securely (SHA-256 hashed). Click "Change Password" to update.</p>
                )}
              </motion.div>
            )}
            ...
            {profile && (
              <motion.div
                ...
                initial={{ opacity: 0 }} transition={{ duration: 0.4, delay: 0.15 }}
              >
                <h2 className="mb-4 text-sm font-semibold text-white">Account Details</h2>
                <div className="grid gap-2 text-xs">
                  {[
                    ["Provider", profile.authProvider === "google" ? "🌐 Google" : "🔒 Local"],
                    ["Member Since", new Date(profile.createdAt).toLocaleDateString("en-IN", { dateStyle: "medium" })],
                    ["Last Updated", new Date(profile.updatedAt).toLocaleDateString("en-IN", { dateStyle: "medium" })],
                  ].map(([k, v]) => (
            ...
            <div className="flex flex-wrap gap-3">
              <button
                className="flex flex-1 items-center justify-center gap-2 rounded-2xl border border-yellow-400/40 bg-yellow-400/12 py-3.5 text-sm font-semibold text-yellow-300 transition hover:bg-yellow-400/20 disabled:opacity-50"
                disabled={loading}
                type="submit"
              >
              ...
              {!isNew && (
                <button
                  className="flex items-center gap-2 rounded-2xl border border-white/12 px-5 py-3.5 text-sm text-white/60 transition hover:border-white/25 hover:text-white"
                  onClick={handleReset}
                  type="button"
                >
            ...
            {/* Google Sign-In */}
            <button
              className="flex w-full items-center justify-center gap-3 rounded-2xl border border-white/15 bg-white/[0.04] py-3.5 text-sm font-semibold text-white/80 transition hover:border-white/30 hover:bg-white/[0.08]"
            ...
            >
              ...
              {profile?.authProvider === "google" ? "Connected with Google" : "Continue with Google"}
            </button>

            {profile && (
              <button
                className="w-full rounded-2xl border border-red-400/20 py-3 text-sm text-red-400/70 transition hover:border-red-400/40 hover:text-red-300"
                onClick={() => { localStorage.removeItem(PROFILE_KEY); window.location.href = "/"; }}
                type="button"
              >
